...
const renderStatusHelp=()=>`<details class="status-help"><summary>상태·색상 안내</summary><dl>${progressGroups.map(([key,title,color])=>`<div><dt><i style="background:${color}" aria-hidden="true"></i>${title}</dt><dd>${statusHelp[key]}</dd></div>`).join('')}</dl><p>막대는 실행 카드 수 기준이며 조율 카드는 제외합니다. 마지막 보고 시각은 작업 상태와 별개이며, 내가 답할 질문은 ‘내 결정 필요’에서 확인합니다.</p></details>`;
...
export function boardProgressCounts(board){
 const cards=[...new Map((board.cards??[]).filter(isExecution).map(c=>[c.key,c])).values()],counts=Object.fromEntries(progressGroups.map(([key])=>[key,0]));
 for(const c of cards)counts[progressGroup(c.displayState)]++;
 return {total:cards.length,counts};
}
export function renderBoardProgress(center,{briefs,showCards=true,showWatch=true}={}){
