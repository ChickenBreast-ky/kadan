...
function activeExecutions(cards, entries, parents, works, excludeWaiting) {
  ...
  for (const entry of projected) {
    if (!entry.role || !entry.taskId || entry.transport === 'mailbox') continue;
...
export function buildSupervisorScope(cards, entries, parents = new Map(), works = []) {
  ...
  const sent = new Set(workEntries(entries).filter(e => e.kind === 'send' && e.taskId && e.transport !== 'mailbox').map(e => e.taskId));
  for (const work of works) {
    if (work.status !== 'open' || !work.executions.some(e => sent.has(e.key.split('/').at(-1)))) continue;
