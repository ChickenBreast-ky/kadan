...
export class ProgressWatch {
  ...
  select({cards,entries,observations,now,enabled,blockedSessions=new Set()}) {
    ...
    for(const card of cards){
      const role=effectiveCardRole(card,entries),session='kadan-'+role,seen=observations.get(session);
      if(card.status!=='assigned'||card.workType==='coordination'||card.activity!=='running'||!role||superRole(role)||!seen?.alive||!seen.screen||seen.expectedPid!=null&&String(seen.pid)!==String(seen.expectedPid)||!openTaskIds(entries,role).includes(card.id))continue;
      ...
      if(enabled&&!selected&&!blockedSessions.has(session)&&now-s.at>=INTERVAL&&now-(this.lastJudged.get(session)??-Infinity)>=INTERVAL){
        selected={source:'progress',role,session,taskId:card.id,input:{title:card.title,scope:card.scope,goal:card.body?.slice(0,6000),
